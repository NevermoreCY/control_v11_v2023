...
def main():
    ...
    valid_path = "BLIP2_split_by_count_V4.json"
    ...
    for j in tqdm(range(len(folders))):
        ...
        folder = folders[j]
        meta_path = img_folder + "/" + folder + "/objarverse_BLIP_metadata_v4.json"
        ...
        # case when tag maching is found in one text:
        # we do a double check
        else:
            text_emb = model_clip.encode(texts)  # 12 x D_embd
            cos_scores = util.cos_sim(text_emb, text_emb)  # 12 x 12
            best_text_0, count_0 = find_best_text(cos_scores, texts, thresh=0.85)
            best_text_0_s = best_text_0.split()
            keep_best_text =False
            for tag in tags:
                if tag[-1] == 's':
                    tag_alt = tag[:-1]
                else:
                    tag_alt = tag + 's'
                if (tag in best_text_0_s or tag_alt in best_text_0_s):
                    keep_best_text = True

            if not keep_best_text:
                best_text = best_text_0
                count = count_0


        meta_data['best_text_v5'] = best_text
        meta_data['text_count_v5'] = count

        out_put_data[str(count)].append(folder)
        out_text_name = img_folder + "/" + folder + "/BLIP_best_text_v3.txt"

        with open(out_text_name, 'w') as f:
            f.write(best_text)

        new_meta_path = img_folder + "/" + folder + "/objarverse_BLIP_metadata_v5.json"
        with open(new_meta_path, 'w') as f:
            json.dump(meta_data, f)


    # s = 1
    out_path = "BLIP2_split_by_count_recheck_tag_V5.json"
